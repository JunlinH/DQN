...
Original file is located at
    https://colab.research.google.com/drive/19LW9oJHLUwv54FSh-2J_KdoclLANmfMT
"""
...
class DqnAgent:
    ...
    def __init__(self, env, action_size):
        ...
        self.update_model()
        self.optimizer = tf.keras.optimizers.Adam(learning_rate=0.0001, beta_1=0.9, beta_2=0.999, epsilon=1e-07)
        self.held_out_memory = self.get_held_out_states(env)
        self.epsilon = 1.0  
        ...
        self.replay_start_size = 10000 ## replay start size; 50,000 in the paper
        self.target_update_steps = 1000 ## target network update frequency; 10,000 in the paper
    ...
    def get_held_out_states(self, env):
        play_steps = 0
        all_states_seen = []
        while play_steps < 100000:
            env.reset()
            done = False
            while not done:
                play_steps += 1
        ...
        held_out_states = random.sample(all_states_seen, 10000)
        held_out_states = tf.Variable(held_out_states, dtype=tf.float32)
        save_pickle("/content/gdrive/My Drive/DQN/DQN_Pong/Memory/held_out_memory.pickle", held_out_states)
        print('saved!')
        self.do()
        return held_out_states
    ...
    while True:
        ...
        while not done:
            ...
            if checkpoint_tracing == agent.checkpoint:
                ...
                save_file('/content/gdrive/My Drive/DQN/DQN_Pong/Metric/average_action_value_record.txt', average_action_value_record)
                #### checkpoint ####
                agent.save('/content/gdrive/My Drive/DQN/DQN_Pong/Checkpoint/check_point_{}.pickle'.format(total_updated_number/agent.checkpoint))
